...
3. **Fetch Full Content**: Use `web_search` to read important sources in full, not just snippets
...
Use `web-fetch` to read full content when:
- You need realtime relevant information
...
Use `web-fetch` to read full content when:
- A search result looks highly relevant and authoritative (usually fetching a link)
