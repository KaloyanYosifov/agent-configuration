---
name: excalidraw
description: Generate hand-drawn style diagrams, flowcharts, and architecture diagrams as PNG images from Excalidraw JSON. Use when user explicitly states that they want a diagram to be generated
---
...
Check if there is a `node_modules` and `fonts` folder in `<skill_dir>/scripts`. If not run `<skill_dir>/scripts/setup.sh` to install dependencies
